# EE6003 Robotic Arm — Controller Wiring

Wiring guide for the handheld controller (`src/controller_uno_r4/`). Two KY-023 joysticks + one 10 kΩ rotary potentiometer drive four MG996R servos on the arm side via the HC-05 Bluetooth pair.

The power architecture (4× AA pack for servos, separate USB power for logic, common ground, HC-05 RX divider) is unchanged from the original arm-side wiring — see the inline comments at the top of `src/arm_mega/main.cpp` for the Mega-side wiring map, and the HC-05 gotcha in §5 below for the controller-side specifics.
...
1. Open Serial Monitor at 115200 baud — you should see `[ctl] boot OK — sticks=shoulder/elbow, SW=base spin, pot=claw`.
...
## 7. Tuning knobs (in `controller_uno_r4/main.cpp`)

| Constant       | Default | What it changes                                            |
|----------------|---------|------------------------------------------------------------|
| `MAX_RATE`     | 60 °/s  | How fast shoulder/elbow move at full joystick deflection (matches arm-side slew cap) |
| `JOY_EXPO`     | 0.6     | Joystick curve strength — 0 linear, 1 pure cubic           |
